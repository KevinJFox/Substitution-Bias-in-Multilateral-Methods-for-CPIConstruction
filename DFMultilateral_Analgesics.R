...
###### Exercises on scanner data: Analgesics
###### From Dominick's supermarket Database
###### https://www.chicagobooth.edu/research/kilts/datasets/dominicks


###### Outputs from this script: Results using Analgesics data
###### 
###### Table A: df_a8
###### Latex code for making table: tbl_a8_latex.txt
###### Data for plotting using in Multilateral_Fig4.R:df_a8_analgesics.xlsx
######
###### Figures: 
###### Figure_analgesics: Plots FCH TCH WTPD GK GEKS CCDI
###### Figure_analgesics_sim: Plots FCH WTPD GK AL LQ CCDI
...
all(min(analgesics$month):max(analgesics$month) %in% unique(analgesics$month))

# Recalling the definitions of the MOVE and QTY variables, we now create our Quantity variable
analgesics$Quantity <- analgesics$MOVE/analgesics$QTY

# Compute the unit values by month
analgesics_monthly <- analgesics %>% mutate(value=PRICE*Quantity) %>% group_by(month, UPC) %>%
...
count_UPC <- table(analgesics_monthly$UPC)
match_UPC <- count_UPC[count_UPC==100]
analgesics_monthly_match <- analgesics_monthly[as.character(analgesics_monthly$UPC) %in% names(match_UPC),]
...
# price index output: df_a8
p_ft_ch <- pb_index(analgesics_monthly_match, p="unit_value", qty="q_sum", id="UPC", tm="month", 
                    type=c("f","t"), seq=c("ch","fb"))[, c(1, 2, 4, 3, 5)]
# GEKS, CCDI, weighted time product dummy, and GK indexes
p_multi <- pm_index(analgesics_monthly_match, p="unit_value", qty="q_sum", id="UPC", tm="month",
                    type=c("wtpd", "gk", "geks", "ccdi"))
# AL indexes
p_al <- sim_index(analgesics_monthly_match, p="unit_value", qty="q_sum", id="UPC", tm="month", type="f", sim="al")
# LQ indexes
p_lq <- sim_index(analgesics_monthly_match, p="unit_value", qty="q_sum", id="UPC", tm="month", type="t", sim="lq")
# combine columns
df_a8 <- data.frame(p_ft_ch, p_multi, p_al, p_lq)[, c(-6,-11, -13)]


###--------------------- LaTex Tables ------------------------------- ###
### Table A8:
### "Alternative Price Levels for Different Methods, Analgesics"
# general framework
tbl_a8 <- xtable(df_a8, caption = "Alternative Price Levels for 
                 Different Methods, Analgesics",
                 latex.environments="center")
# column alignment
align(tbl_a8) <- rep("r", 12)
# column names
names(tbl_a8) <- c("$t$", "$\\pi_{FCH}^{t}$", "$\\pi_{TCH}^{t}$", 
                   "$\\pi_{FFB}^{t}$", "$\\pi_{TFB}^{t}$", 
                   "$\\pi_{WTPD}^{t}$", "$\\pi_{GK}^{t}$", "$\\pi_{GEKS}^{t}$",
                   "$\\pi_{CCDI}^{t}$", "$\\pi_{AL}^{t}$", "$\\pi_{LQ}^{t}$")
# number of digits
digits(tbl_a8) <- c(0,0,rep(4,10))
tbl_a8_latex <- capture.output(print(tbl_a8, include.rownames = FALSE,
                                     sanitize.text.function=function(x){x},
                                     caption.placement = "top",
                                     tabular.environment = "longtable",
                                     floating = FALSE,
                                     hline.after=c(-1)))
write(tbl_a8_latex, "tbl_a8_latex.txt")
...
# data source: df_a8
#%%%%%%%%%%
pdf("Figure_analgesics.pdf")
plot(df_a8[,c("month","f_ch")], ylim=c(0.98, 1.42),
     ylab="Index", xlab="Month", type="l", col=2, lwd=2, cex.main=1.5)
lines(df_a8[,c("month","t_ch")], col=3, lwd=2)
lines(df_a8[,c("month","wtpd")], col=4, lwd=2)
lines(df_a8[,c("month","gk")], col=6, lwd=2)
lines(df_a8[,c("month","geks")], col="violetred", lwd=2)
lines(df_a8[,c("month","ccdi")], col="gold", lwd=2)
...
pdf("Figure_analgesics_sim.pdf")
plot(df_a8[,c("month","f_ch")], col=2, ylim=c(0.98, 1.42),
     ylab="Index", xlab="Month", type="l", lwd=2, cex.main=1.42)
lines(df_a8[,c("month","wtpd")], col=4, lwd=2)
lines(df_a8[,c("month","gk")], col=6, lwd=2)
lines(df_a8[,c("month","al")], col="violetred", lwd=2)
lines(df_a8[,c("month","lq")], col=3, lwd=2)
lines(df_a8[,c("month","ccdi")], col="gold", lwd=2)
...
### WRITE OUT DATA FOR USE IN A COMBINED PLOT: DFMulti_Fig4_2020
write.xlsx(df_a8, "df_a8_analgesics.xlsx")
